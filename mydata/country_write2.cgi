sub COUNTRY_WRITE2{
	...
	if($lockkey) { &F_LOCK; }
	open(IN,"$BBS_LIST2") or &ERR2('파일을 열지 않았습니다. err no :country');
	@BBS_DATA1 = <IN>;
	close(IN);
	...
	if($in{"type"} eq "all"){$bbtype = 1;$back = "COUNTRY_ALL_TALK"}else{$bbtype = 0;$back = "COUNTRY_TALK2"}
	...
	open(OUT,">$BBS_LIST2") or &ERR('파일을 열지 않았습니다.');
	print OUT @BBS_DATA1;
	close(OUT);
