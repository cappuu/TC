     ...
     for($i=0;$i<12;$i++){
		...
		for($j=0;$j<12;$j++){
				...
				if($m_hit){
					...
					}elsif($zzname eq "서량" || $zzname eq "천수" || $zzname eq "상용" || $zzname eq "졸본" || $zzname eq "한중" || $zzname eq "강주" || $zzname eq "강하" || $zzname eq "장사" || $zzname eq "남해" || $zzname eq "진유" || $zzname eq "시상" || $zzname eq "초" || $zzname eq "단양" || $zzname eq "북평" || $zzname eq "회계" || $zzname eq "서라벌" || $zzname eq "사비"){
